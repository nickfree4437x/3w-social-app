...
const Feed = () => {
  ...
  const fetchPosts = async (pageNo = 1, append = false) => {
    try {
      ...
      const res = await axios.get(
        `https://threew-social-app-jf3q.onrender.com/api/v1/posts?page=${pageNo}&limit=5`,
        {
          headers: {
  ...
  const handlePostCreated = (newPost) => {
    console.log("Post created callback called", newPost);
    setPosts((prev) => [newPost, ...prev]);
    // Toast removed from here to avoid duplicate
  };
